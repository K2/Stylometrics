...
import type { QuoteStyleOptions } from './quote_style_carrier.mjs';
...
export class StylometricToolkit {
    ...
    private changePointDetector: StyloCPADetector;
    private carrier: StylometricCarrier;
    private quoteStyleCarrier: QuoteStyleCarrier;
    ...
    constructor() {
        ...
        this.changePointDetector = new StyloCPADetector();
        this.carrier = new StylometricCarrier();
        this.quoteStyleCarrier = new QuoteStyleCarrier();
    }
    ...
    analyzeText(text: string): TextAnalysisResult {
        ...
        const aiResult = this.classifier.classify(text);
    ...
    /**
     * Embed payload using only quotation mark style changes
     * 
     * @param text Text with quotation marks
     * @param payload Bitstream to embed
     * @param options Quote style options
     * @returns Modified text with embedded bits
     */
    embedWithQuoteStyles(
        text: string, 
        bits: boolean[],
        options: QuoteStyleOptions = {}
    ): string {
        // Create quote style carrier with specified options
        const carrier = new QuoteStyleCarrier(options);
        
        // Encode bits in text
        const result = carrier.encode(text, bits);
        
        return result.modifiedText;
    }
    
    /**
     * Extract payload from text using quotation mark style changes
     * 
     * @param text Text with embedded bits
     * @param expectedBits Number of bits to extract
     * @param options Quote style options (must match embedding options)
     * @returns Extracted bits
     */
    extractFromQuoteStyles(
        text: string, 
        expectedBits: number,
        options: QuoteStyleOptions = {}
    ): boolean[] {
        ...
        // Extract bits from text
        return carrier.extract(text, expectedBits);
    }
...
export function demonstrateToolkit(
    ...
    // 4. Quote style carrier demo
    console.log("\n4. QUOTE STYLE CARRIER DEMO:");
    console.log("---------------------------");
    
    // Only run this if the text contains quotes
    if (humanText.includes('"') || humanText.includes("'")) {
        try {
            // Convert short payload to bits
            const shortPayload = "secret";
            const bits = toolkit.textToBits(shortPayload);
            
            // Embed using quote styles
            const quoteModifiedText = toolkit.embedWithQuoteStyles(
                humanText, 
                bits, 
                { useSmartQuotes: true }
            );
            
            console.log(`Embedding "${shortPayload}" using quote styles`);
            console.log("\nModified text (first 100 chars):");
            console.log(quoteModifiedText.substring(0, 100) + "...");
            
            // Extract from quote styles
            const extractedBits = toolkit.extractFromQuoteStyles(
                quoteModifiedText, 
                bits.length,
                { useSmartQuotes: true }
            );
            const extractedQuotePayload = toolkit.bitsToText(extractedBits);
            
            console.log(`\nExtracted payload: "${extractedQuotePayload}"`);
            console.log(`Recovery successful: ${extractedQuotePayload === shortPayload ? 'YES' : 'NO'}`);
        } catch (error) {
            console.error(`Error in quote style carrier demo: ${error.message}`);
        }
    } else {
        console.log("Input text does not contain quotes for demonstration");
    }
